...
p2 <- gravity_test3 %>%
  ...
  stat_chull(data = xy1 %>%
               dplyr::group_by(type) %>%
               slice(5383:5408), aes(x = xcoord, y = ycoord), alpha = 0.1, fill = "blue", geom = "polygon") +
  stat_chull(data = xy2 %>%
               dplyr::group_by(type) %>%
               slice(5383:5408), aes(x = xcoord, y = ycoord), alpha = 0.1, geom = "polygon") +
  geom_point(aes(x = p1_x, y = p1_y), color = "blue", size = 4) +
...
gganimate::animate(p2, start_pause = 10, end_pause = 10, nframes = 700, renderer = ffmpeg_renderer())
